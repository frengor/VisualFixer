...
public class Kelp implements Listener {
	...
	@EventHandler
	public void onKelpBreak(BlockBreakEvent e) {

		if (!e.isCancelled() || !e.getBlock().getType().toString().contains("KELP"))
			return;
	...
	private class Advanced implements Listener {
		...
		public void onKelpBreak(PlayerInteractEvent e) {

			if (!e.isCancelled() || e.getAction() != Action.LEFT_CLICK_BLOCK
					|| !e.getClickedBlock().getType().toString().contains("KELP"))
				return;
